...
# ##******************************linear gru**********************    
    sigmoid_gi = torch.sigmoid(gi)
    sigmoid_der_gi = sigmoid_gi * (1-sigmoid_gi)
    
    g_r, g_z, g_n = sigmoid_gi.chunk(3, 1)
    f_r, f_z, f_n = sigmoid_der_gi.chunk(3, 1)
...
#     #hidden = g + f_n*(1-g_z)*g_r*h_n-g_z*g_r*h_i+g_z*hidden+f_r*h_r###########improved
    #hidden = g + f_n*(1-g_z)*g_r*h_n-f_z*g_n *h_i+f_r*g_z*g_r*h_r+g_z*hidden#variant 5
#     hidden = g + g_z*h_n ######################degenerated##########
# # #******************************variant GRU*****************************************#
#     resetgate = torch.sigmoid(i_r+h_r) #(i_r + h_r)*0.25+0.5-(i_r + h_r)**3*(0.6/48)
