...
class data_generator:
    def __init__(self, config, data_path, is_training=True, is_sort=False):
        '''
        ...
        self.data_batch = self.data_backup#copy.deepcopy(self.data_backup)
        if is_sort:
            self.data_batch = self.sort_filter_data(self.data_batch)
...
#             self.elmo = Elmo(options_file, weight_file, 2, dropout=0)
    ...
    def sort_filter_data(self, data, max_len = None):
        '''
        Sort the instances based on the lengths of source instances
        '''
        lens = [len(item) for item in data]
        temp = list(zip(lens, data))
        temp.sort()
        sort_lens, sort_data = list(zip(*temp))
        if max_len:
            index = 0
            for i, length in enumerate(sort_lens):
                if length >= max_len:
                    index = i
                    break
